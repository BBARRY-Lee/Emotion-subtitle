...
def calc_label_accuracy(X,Y):
    ...
    label = [0 for _ in range(4)] #실제 dataset의 각 레이블당 갯수 
...
def sentence_train(model, train_dataloader, val_dataloader, learning_rate, epochs):
    ...
    #            '/home/ubuntu/model/pre-trained/230209_klueBert_CrossVal.png')    
...
def run_classifier():
    ...
    model.load_state_dict(torch.load('/home/ubuntu/model/pre-trained/230209_klueBert_crossVal.pt', map_location = device))
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train[sent_col], train[label_col])):
        ...
        model = sentence_train(model, train_dataloader, val_dataloader, CFG['LEARNING_RATE'], CFG['EPOCHS'])
        
    torch.save(model.state_dict(), '/home/ubuntu/model/pre-trained/klueBert_crossVal_epoch40.pt')
